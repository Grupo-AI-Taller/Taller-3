...
def crear_kb() -> KnowledgeBase:
    ...
    # dra santos y director vega tienen documentos que prueban que no estaban ahi
    kb.add_fact(Predicate("documentacion_viaje", (dra_santos,)))
    ...
    # los registros de acceso muestran que ambos entraron el sabado
    ...
    # rios recibia pagos de syntek, la empresa que gana con el sabotaje
    kb.add_fact(Predicate("recibio_pagos", (tec_rios, syntek_corp)))
    kb.add_fact(Predicate("beneficiario_sabotaje", (syntek_corp,)))
    kb.add_fact(Predicate("lugar_sabotaje", (sala_cultivos,)))
    kb.add_fact(Predicate("denuncia", (asistente_mora, tec_rios)))

    # documentos oficiales de viaje o conferencia cuentan como coartada
    kb.add_rule(Rule(
        head=Predicate("coartada_verificada", (Term("$X"),)),
        body=(Predicate("documentacion_viaje", (Term("$X"),)),),
    ))
    kb.add_rule(Rule(
        head=Predicate("coartada_verificada", (Term("$X"),)),
    ...
    # recibir plata de quien se beneficia del crimen es conflicto de intereses
    kb.add_rule(Rule(
        head=Predicate("conflicto_intereses", (Term("$X"), Term("$E"))),
        body=(Predicate("recibio_pagos", (Term("$X"), Term("$E"))), Predicate("beneficiario_sabotaje", (Term("$E"),))),
    ))
    kb.add_rule(Rule(
        head=Predicate("motivo_economico", (Term("$X"),)),
        body=(Predicate("conflicto_intereses", (Term("$X"), Term("$E"))),),
    ))
    # el registro de acceso prueba que estabas en el lugar cuando ocurrio
    kb.add_rule(Rule(
        head=Predicate("acceso_en_momento", (Term("$X"),)),
        body=(Predicate("acceso_registrado", (Term("$X"), Term("$L"))), Predicate("lugar_sabotaje", (Term("$L"),))),
    ))
    # para ser culpable necesitas las tres cosas: sin coartada, motivo y haber estado ahi
    kb.add_rule(Rule(
    ...
    # si el que denuncia tambien estuvo ahi, su denuncia tiene mas peso
    kb.add_rule(Rule(
        head=Predicate("denuncia_informada", (Term("$X"), Term("$Y"))),
        body=(Predicate("denuncia", (Term("$X"), Term("$Y"))), Predicate("acceso_en_momento", (Term("$X"),))),
    ))
